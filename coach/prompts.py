    ...
    return PromptResult(
        messages=[
            Message(
                ...
                f"6. Use update_weekly_plan(plan=...) to save — it takes a structured "
                f"plan dict (not a JSON string), and every non-rest session needs a "
                f"purpose string. The response lists `purpose_warnings` for sessions "
                f"missing one — fix them before pushing to Garmin.\n"
    ...
    return PromptResult(
        messages=[
            Message(
                ...
                f"3. Call query_metrics(kind='intensity') for zone analysis (or "
                f"request snapshot sections=['fitness', 'activities'])\n\n"
                f"## Review checklist\n"
    ...
    return PromptResult(
        messages=[
            Message(
                ...
                f"4. If they have races, use races(action='add', ...) to register them\n"
